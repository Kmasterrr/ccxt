...
class valr extends Exchange {
    ...
    public function describe() {
        return $this->deep_extend(parent::describe(), array(
            ...
            'api' => array( // https://github.com/ccxt/ccxt/blob/master/CONTRIBUTING.md#implicit-api-methods
                'exchange' => array(
                    'get' => array(
                        'markets' => 1,
                    ),
    ...
    public function fetch_markets($params = array ()) {
        return Async\async(function () use ($params) {
            ...
            // )]
            $result = array();
            $markets = $this->safe_value($response, array());
            for ($i = 0; $i < count($markets); $i++) {
                ...
                $status = $this->safe_string($market, 'active');
                $result[] = array(
                    'id' => $id,
                    'symbol' => $base . '/' . $quote,
                    'base' => $base,
                    ...
                    'precision' => array(
                        'amount' => null,
                        'price' => $this->parse_number($this->parse_precision($this->safe_string($market, 'baseDecimalPlaces'))),
                    ),
                    'limits' => array(
                        'leverage' => array(
                            'min' => null,
                        ...
                        'amount' => array(
                            'min' => $this->safe_number($market, 'minQuoteAmount'),
                            'max' => $this->safe_number($market, 'maxQuoteAmount'),
                        ),
                        'price' => array(
                            'min' => $this->safe_number($market, 'minBaseAmount'),
                            'max' => $this->safe_number($market, 'maxBaseAmount'),
                        ),
                        'cost' => array(
                            'min' => null,
                            'max' => null,
    ...
    public function fetch_accounts($params = array ()) {
        return Async\async(function () use ($params) {
             ...
             * @param {array} $params extra parameters specific to the valr api endpoint
    ...
    public function fetch_order($id, $symbol = null, $params = array ()) {
        return Async\async(function () use ($id, $symbol, $params) {
             ...
             * @param {string|null} $symbol not used by valr fetchOrder
             * @param {array} $params extra parameters specific to the valr api endpoint
